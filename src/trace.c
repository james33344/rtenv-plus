...
unsigned int get_reload() { return *(unsigned int *) 0xE000E014; }

unsigned int get_current() { return *(unsigned int *) 0xE000E018; }
...
void trace_task_create(void *task, const char *task_name,
...
void trace_task_switch(void *prev_task, unsigned int prev_tick,
                       void *curr_task) {
    int sub = prev_tick - get_current();
    char buf[128];
    int len = snprintf(buf, 128, "switch %d %d %d %d %d %d %d\n", prev_task,
                       curr_task, tick_count, get_reload(), prev_tick,
                       get_current(), sub);
    host_action(SYS_WRITE, logfile, buf, len);
}
...
void trace_queue_send(void *task, void *queue) {
    char buf[128];
    int len =
        snprintf(buf, 128, "queue send %d %d %d\n", get_time(), task, queue);
    host_action(SYS_WRITE, logfile, buf, len);
}
...
void trace_queue_recv(void *task, void *queue) {
    char buf[128];
    int len =
        snprintf(buf, 128, "queue recv %d %d %d\n", get_time(), task, queue);
    host_action(SYS_WRITE, logfile, buf, len);
}
...
void trace_queue_block(void *task, void *queue) {
    char buf[128];
    int len =
        snprintf(buf, 128, "queue block %d %d %d\n", get_time(), task, queue);
    host_action(SYS_WRITE, logfile, buf, len);
}
