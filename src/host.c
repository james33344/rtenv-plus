...
int host_system(va_list v1) {
    ...
    return host_call(SYS_SYSTEM, (param[]){{.pdChrPtr = tmpChrPtr},
                                           {.pdInt = strlen(tmpChrPtr)}});
}
...
int host_open(va_list v1) {
    ...
    tmpChrPtr = va_arg(v1, char *);
    return host_call(SYS_OPEN, (param[]){{.pdChrPtr = tmpChrPtr},
                                         {.pdInt = va_arg(v1, int)},
                                         {.pdInt = strlen(tmpChrPtr)}});
}
...
int host_write(va_list v1) {
    return host_call(SYS_WRITE, (param[]){{.pdInt = va_arg(v1, int)},
                                          {.pdPtr = va_arg(v1, void *)},
                                          {.pdInt = va_arg(v1, int)}});
}
