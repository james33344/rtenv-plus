...
int regfile_request_readable(struct regfile *regfile,
    ...
    if (regfile->request_pid == 0) {
        ...
        struct fs_request fs_request = {.cmd = FS_CMD_READ,
                                        .from = task->pid,
...
int regfile_request_writable(struct regfile *regfile,
    ...
    if (regfile->request_pid == 0) {
        ...
        struct fs_request fs_request = {.cmd = FS_CMD_WRITE,
                                        .from = task->pid,
                                        .target = regfile->file.fd,
                                        .size = size,
                                        .pos = regfile->pos};
...
int regfile_init(int fd, int driver_pid, struct file *files[],
    ...
    event_monitor_register(monitor, regfile->event, regfile_event_release,
                           files[fd]);
...
int regfile_response(int fd, char *buf, int len) {
    struct regfile_response response = {.transfer_len = len, .buf = buf};
