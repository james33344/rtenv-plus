...
int file_mknod(int fd, int driver_pid, struct file *files[], int dev,
    ...
    switch (dev) {
        case S_IFIFO:
            result =
                fifo_init(fd, driver_pid, files, memory_pool, event_monitor);
            break;
        case S_IMSGQ:
        ...
        case S_IFBLK:
            result =
                block_init(fd, driver_pid, files, memory_pool, event_monitor);
            break;
        case S_IFREG:
            result =
                regfile_init(fd, driver_pid, files, memory_pool, event_monitor);
            break;
        default:
            result = -1;
    }
