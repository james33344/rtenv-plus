...
#define get_pthread_attr_priority(attr) ({ attr->sched_param.sched_priority; })

#define is_thread_not_alive(thread) ({ thread->released == 0 ? 0 : 1; })
...
pthread_t pthread_self() { return *__pthread_record[current_tcb->pid]; }
...
int pthread_join(pthread_t thread, void **value_ptr) {
    ...
    if (!is_thread_value_legal(&thread))
        return EINVAL;

    if (!is_attr_joinable(&thread))
        return EINVAL;
