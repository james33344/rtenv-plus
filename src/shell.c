...
const hcmd_entry cmd_data[CMD_COUNT] = {
        ...
        [CMD_HISTORY] = {.cmd = "history",
                         .func = show_history,
                         .description = "Show latest commands entered."},
        [CMD_MAN] = {.cmd = "man",
                     .func = show_man_page,
                     .description = "Manual pager."},
        [CMD_PS] = {.cmd = "ps",
                    .func = show_task_info,
        ...
        [CMD_PWD] = {.cmd = "pwd",
                     .func = show_pwd,
                     .description = "Show current mount point."},
        [CMD_LS] = {.cmd = "ls", .func = show_ls, .description = "Show files."},
        [CMD_CAT] = {.cmd = "cat",
                     .func = show_cat,
                     .description = "Show files content."},
};
...
void queue_str_task1() { queue_str_task("Hello 1\n", 200); }

void queue_str_task2() { queue_str_task("Hello 2\n", 50); }
...
void show_cmd_info(int argc, char *argv[]) {
    ...
    for (i = 0; i < CMD_COUNT; i++) {
        ...
        write(fdout, ": ", 3);
        write(fdout, cmd_data[i].description,
              strlen(cmd_data[i].description) + 1);
        write(fdout, next_line, 3);
    }
}
...
void show_history(int argc, char *argv[]) {
    ...
    for (i = cur_his + 1; i <= cur_his + HISTORY_COUNT; i++) {
        if (cmd[i % HISTORY_COUNT][0]) {
            write(fdout, cmd[i % HISTORY_COUNT],
                  strlen(cmd[i % HISTORY_COUNT]) + 1);
            write(fdout, next_line, 3);
        }
    }
}
