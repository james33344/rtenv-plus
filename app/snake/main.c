...
int btn_clicked(){
  ...
  else if ((TP_State->X <= LCD_PIXEL_WIDTH/2) && (TP_State->X > LCD_PIXEL_WIDTH/4)) {
		return To_ry;
	}
  else if ((TP_State->X <= LCD_PIXEL_WIDTH/4 * 3) && (TP_State->X > LCD_PIXEL_WIDTH/2)) {
		return To_y;
	}
  else if ((TP_State->X <= LCD_PIXEL_WIDTH) && (TP_State->X > LCD_PIXEL_WIDTH/4 * 3)) {
		return To_x;
	}
...
while(1) {
  ...
  LCD_SetTextColor(LCD_COLOR_WHITE); 
	LCD_DrawFullRect(snake[snake_count-1].x, snake[snake_count-1].y, snake_size, snake_size);
  ...
  if(rc!=0) {
	...
	if(!is_over) {
		...
		for(int i = 0; i < snake_count; i++) {
			LCD_DrawFullRect(snake[i].x, snake[i].y, snake_size, snake_size);
		}
...
//	LCD_DrawRect(40,40,40,40);
    ...
    LCD_DisplayStringLine(LINE(10), (uint8_t*)"    main init finished");
		...
		LCD_DisplayChar(LINE(35), LCD_PIXEL_WIDTH/4 * 3 - DIR_OFFSET, (uint8_t) 'D');
    ...
    LCD_DisplayStringLine(LINE(11), (uint8_t*)"    touch screen to start");
       ...
       LCD_ClearLine(LINE(linenum));
			...
			for(int i=0;i<snake_count;i++) {
				LCD_DrawFullRect(snake_x_pos, snake_tail_pos + snake_size * i, snake_size, snake_size);
